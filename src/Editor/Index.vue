    ...
    methods: {
      init () {
        ...
        _t.editor.on('editor:setItem', function (data) {
          ...
          data.forEach((item, index) => {
            ...
            console.log(model, shapeItem)
            if (!index) {
              // 更新第一个节点
              console.warn(shapeItem, model)
      ...
      doAddNode (info) {
        ...
        const node = {
          ...
          draggable: true,
          type: info.type,
          isNeuralNetworkModel: info.isNeuralNetworkModel || false,
        ...
        if (node.isNeuralNetworkModel) {
          node.neuralNetworkData = info.neuralNetworkData
        }
