    ...
    <Form v-if="isNode || isEdge" label-position="top">
      ...
      <CardItem v-if="isNeuralNetworkModel" :title="$t('L10106')">
        <div class="form-item-block">
          <div v-for="param in formData.neuralNetworkData" :key="param.name" class="row">
            <FormItem class="col" :required="param.required" :label="param.name">
              <InputNumber
                v-if="param.type == 'int'"
                class="input-number"
                v-model="param.value"
                :step="1"
                @on-change="val => handleChange(val, param.name)">
              </InputNumber>
              <InputNumber
              ...
              <Select v-else-if="param.type == 'bool'" v-model="param.value" size="small" @on-change="val => handleChange(val, param.name)">
                <Option value="True">True</Option>
                <Option value="False">False</Option>
              </Select>
              <Input v-else v-model="param.value" icon="" :placeholder="param.name" @on-change="val => handleChange(val, param.name)">
                <template slot="suffix">
                  <Tooltip placement="left" :content="'参数类型：' + param.type">
  ...
  export default {
        ...
        handler (val) {
          ...
          if (_t.firstItem) {
            ...
            formData.labelStyle = {
              ...
              lineWidth: labelCfg.style.lineWidth | 0.1,
    ...
    methods: {
      handleChange (val, from) {
        ...
        if (_t.formData.isNeuralNetworkModel) {
          model.isNeuralNetworkModel = true
          model.neuralNetworkData = _t.formData.neuralNetworkData
        }
        if (_t.isNode) {
          model = {
            ...model,
