 ...
 * 物料配置
 */
import torchModel from './torchModel.json'
...
export default function (enableMaterials) {
  ...
  */
  ...
  for (const name in torchModel) {
    defMaterials[defMaterials.length - 1].children.push({
      type: 'rectangle',
      label: name,
      defaultLabel: name,
      enable: true,
      ...
      anchorPoints: anchorPoints,
      shapeControl: shapeControl,
      isNeuralNetworkModel: true,
      modelName: name,
      neuralNetworkData: torchModel[name],
      icon: `<rect x="0" y="0" width="100%" height="100%" fill="#ffffff" stroke="#000000" stroke-width="1.3" pointer-events="all"></rect>
      <text x="50%" y="50%" style="text-anchor: middle;dominant-baseline: middle;">` + name + `</text>`
    })
  }
  defMaterials[defMaterials.length - 1].children.sort((first, second) => {
    return first.defaultLabel.localeCompare(second.defaultLabel)
  })
  let materials = []
