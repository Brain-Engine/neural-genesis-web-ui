...
export default {
  ...
  options: {
    ...
    onCanvasMouseenter (event) {
      ...
      const _t = this
      if (_t.info && _t.info.type === 'dragNode' && _t.dragNode.status === 'dragNodeToEditor') {
        _t[_t.info.type].createDottedNode.call(_t, event)
      }
    },
    onCanvasMouseleave (event) {
      const _t = this
      if ((!_t.dragNode.status && _t.info && _t.info.type === 'dragNode') || (_t.dragNode.status && event.originalEvent.buttons === 0 && !utils.common.isInG6Area(event))) {
        _t[_t.info.type].stop.call(_t, event)
      }
    },
      ...
      move (event) {
        ...
        if (_t.dragNode.status === 'dragNodeToEditor') {
          ...
          if (_t.info.node) {
            ...
            const { id, groupId } = _t.info.node.getModel()
            _t.graph.find('node', node => {
              const model = node.getModel()
              // 更新当节点
              // console.log(id, groupId, model.id, model.id === id)
              if (model.id === id) {
                const attrs = {
                  // 处理点击移动 图形时的抖动
                ...
                if (groupId && model.groupId && model.groupId === groupId) {
                  // 更新同组节点
                  if (_t.info.offset.model[model.id] === undefined) {
                    _t.info.offset.model[model.id] = {
                      x: model.x,
                      y: model.y
                    }
                  }
                  const offset = _t.info.offset.model[model.id]
                  _t.graph.updateItem(node, {
                    // x: model.x + event.originalEvent.movementX,
                    // y: model.y + event.originalEvent.movementY
                    x: offset.x + event.x - _t.info.offset.startX,
                    y: offset.y + event.y - _t.info.offset.startY
                  })
                }
              }
            })
